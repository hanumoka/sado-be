...
> **멀티테넌시 전체 구현**: `12_멀티테넌시_설계_가이드.md` 참조
>
> 요약: TenantFilterAspect가 모든 Repository 메서드 전에 Hibernate Filter를 활성화하여 `WHERE tenant_id = ?` 조건을 자동 추가합니다.

**핵심 컴포넌트**:
- `TenantAwareEntity`: Base Entity with `tenant_id` column
- `TenantFilterAspect`: AOP를 통한 자동 Filter 활성화
- `TenantContext`: ThreadLocal 기반 tenant_id 저장
